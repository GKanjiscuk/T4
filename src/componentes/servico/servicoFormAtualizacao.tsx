    ...
    <>
      {step === 1 && (
        <Form.Group className="mb-3">
          <Form.Label style={{ color: azulEscuro }}>ID do Serviço</Form.Label>
          <Form.Control
            type="text"
            value={idPesquisa}
      ...
      {step === 2 && servicoParaAtualizar && (
        <div>
          <h5 style={{ color: azulEscuro, marginBottom: '15px' }}>Dados Atuais do Serviço (ID: {servicoParaAtualizar.id}):</h5>
          <p style={{ color: azulEscuro }}>Preencha os campos que deseja atualizar. Deixe em branco para manter o valor atual.</p>
          <Form.Group className="mb-3">
            <Form.Label style={{ color: azulEscuro }}>Nome do Serviço (Atual: {servicoParaAtualizar.nome})</Form.Label>
            <Form.Control
              name="nome"
              value={servico.nome}
          ...
          <Form.Group className="mb-3">
            <Form.Label style={{ color: azulEscuro }}>Preço (Atual: R$ {parseFloat(servicoParaAtualizar.preco).toFixed(2)})</Form.Label>
            <Form.Control
              name="preco"
              value={servico.preco}
              onChange={handleChange}
          ...
          <Form.Group className="mb-3">
            <Form.Label style={{ color: azulEscuro }}>Consumo (Atual: {servicoParaAtualizar.consumo})</Form.Label>
            <Form.Control
              name="consumo"
      ...
      {step === 2 && !servicoParaAtualizar && (
        <p style={{ color: azulEscuro }}>Serviço não encontrado ou ID não fornecido.</p>
      )}
    </>
  );
};
